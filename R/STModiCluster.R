...
STModiCluster <- function(InDir = InDir,
  ...
  # Adjusted_expr_mtx
  Sys.setenv(RETICULATE_PYTHON = "/mnt/ilustre/users/hao.wei/miniconda3/envs/stlearn/bin/python")
  ...
  # Add ImmuneScore
  if (!is.null(markers)) {
    Normalfeatures <- read.csv(markers, header = FALSE)
    Normalfeatures <- Normalfeatures[,1]
