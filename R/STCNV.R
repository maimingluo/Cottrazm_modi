...
STCNV <- function(TumorST = TumorST,
  ...
  if (!is.null(spots)) {
    spots_list <- read.csv(spots, header = FALSE)
    spots_list <- spots_list[,1]
    TumorST@meta.data[spots_list, "seurat_clusters"] <- 999
    ref_cluster <- 999
  }else {
  ...
  }else if(specie %in% c("mouse", "rat")) {
    gene_order_file <- system.file("extdata/mouse.uniq.sort.bed", package = "Cottrazm")
  }
